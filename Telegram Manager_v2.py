...
class TelegramBotGUI:
    def __init__(self, root):
        ...
        self.awaiting_password = False  # Flag for 2FA password input
    ...
    def setup_connection_tab(self):
        ...
        # Password frame for 2FA
        self.password_frame = Frame(cred_frame)
        self.password_frame.grid(row=4, column=0, columnspan=2, pady=10, sticky=EW)
        
        Label(self.password_frame, text="2FA Password:").pack(side=LEFT, padx=5)
        self.password_entry = Entry(self.password_frame, width=20, show="*")
        self.password_entry.pack(side=LEFT, padx=5)
        self.password_entry.pack_forget()
        
        self.submit_password_btn = Button(self.password_frame, text="Submit Password", 
                                          command=self.submit_2fa_password, bg="blue", fg="white")
        self.submit_password_btn.pack(side=LEFT, padx=5)
        self.submit_password_btn.pack_forget()
    ...
    def show_password_input(self):
        ...
        self.update_connection_status("🔒 Two-factor authentication enabled. Please enter your password.")
        self.log_message("2FA detected - Password required")
    ...
    def submit_verification_code(self):
        ...
        async def verify():
            try:
                ...
                error_msg = str(e)
                if "password" in error_msg.lower() or "two-steps" in error_msg.lower():
                    # Password required
                    self.root.after(0, self.show_password_input)
                    return "password_needed"
                else:
                    self.log_message(f"Verification failed: {error_msg}")
                    return False
        
        def on_verified(result):
            if result == "password_needed":
                self.awaiting_password = True
                self.update_connection_status("✓ Code accepted! Please enter your 2FA password.")
                # Clear verification code input
                self.verification_code.delete(0, END)
                # Hide verification inputs but keep them for potential resend
                self.verification_code.pack_forget()
                self.submit_code_btn.pack_forget()
            elif result:
                self.setup_message_handler()
    ...
    def submit_2fa_password(self):
        ...
        async def verify_password():
            try:
                ...
                self.log_message(f"Password verification failed: {e}")
                return False
        
        def on_password_verified(success):
            if success:
                self.setup_message_handler()
                self.root.after(0, self.on_connect_success)
            else:
                self.root.after(0, lambda: self.on_connect_failure("Invalid 2FA password"))
                # Clear password input
                self.password_entry.delete(0, END)
        
        self.run_async(verify_password(), on_password_verified)
        self.log_message("Verifying 2FA password...")
    ...
    def resend_verification_code(self):
        ...
        def on_resend(success):
            if success:
                ...
                # Show verification input again
                if not self.verification_code.winfo_ismapped():
                    self.verification_code.pack(side=LEFT, padx=5)
                    self.submit_code_btn.pack(side=LEFT, padx=5)
                    self.resend_code_btn.pack(side=LEFT, padx=5)
                    self.verification_code.focus()
